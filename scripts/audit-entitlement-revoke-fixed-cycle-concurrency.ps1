...
$ents=@('7f000000-0000-0000-0000-000000000030','7f000000-0000-0000-0000-000000000031','7f000000-0000-0000-0000-000000000032','7f000000-0000-0000-0000-000000000033')
$series=@('7f000000-0000-0000-0000-000000000040','7f000000-0000-0000-0000-000000000041','7f000000-0000-0000-0000-000000000042','7f000000-0000-0000-0000-000000000043')
$cycles=@('7f000000-0000-0000-0000-000000000050','7f000000-0000-0000-0000-000000000051','7f000000-0000-0000-0000-000000000052')
$orders=@('7f000000-0000-0000-0000-000000000060','7f000000-0000-0000-0000-000000000061','7f000000-0000-0000-0000-000000000062','7f000000-0000-0000-0000-000000000063')
$items=@('7f000000-0000-0000-0000-000000000070','7f000000-0000-0000-0000-000000000071','7f000000-0000-0000-0000-000000000072','7f000000-0000-0000-0000-000000000073')
$events=@('7f000000-0000-0000-0000-000000000080','7f000000-0000-0000-0000-000000000081','7f000000-0000-0000-0000-000000000082','7f000000-0000-0000-0000-000000000083')
$cleanup="begin;set local session_replication_role='replica';delete from public.audit_logs where actor_user_id in('$student','$teacher','$admin');delete from public.fixed_entitlement_cycles where student_user_id='$student';delete from public.recurring_lesson_occurrences where student_user_id='$student';delete from public.recurring_lesson_series where student_user_id='$student';delete from public.lesson_credit_ledger where beneficiary_user_id='$student';delete from public.entitlement_revoke_operations where entitlement_id in('$($ents[0])','$($ents[1])','$($ents[2])','$($ents[3])');delete from public.entitlements where beneficiary_user_id='$student';delete from public.order_item_fulfillment_snapshots where order_item_id in('$($items[0])','$($items[1])','$($items[2])','$($items[3])');delete from public.order_fulfillment_events where order_id in('$($orders[0])','$($orders[1])','$($orders[2])','$($orders[3])');delete from public.order_items where order_id in('$($orders[0])','$($orders[1])','$($orders[2])','$($orders[3])');delete from public.orders where id in('$($orders[0])','$($orders[1])','$($orders[2])','$($orders[3])');delete from public.lesson_package_product_configs where product_id='$product';delete from public.products where id='$product';delete from public.teacher_scheduling_settings where teacher_user_id='$teacher';delete from public.student_teacher_relationships where id='$relationship';delete from public.teacher_public_profiles where public_slug='p16c-race-teacher';delete from public.teacher_profiles where user_id='$teacher';delete from public.public_profiles where user_id in('$student','$teacher','$admin');delete from public.user_roles where user_id in('$student','$teacher','$admin');delete from public.profiles where user_id in('$student','$teacher','$admin');delete from auth.users where id in('$student','$teacher','$admin');commit;"
...
try{
  ...
  $operations=Scalar "select count(*)||'|'||count(*) filter(where status='completed')||'|'||count(*) filter(where result_payload->>'already_revoked'='true') from public.entitlement_revoke_operations where entitlement_id='$($ents[2])'"
  ...
  if($state-eq'revoked|invalidated|null'-and$counts-eq'1|1|1'-and$operations-eq'2|2|1'){"C [PASS P1-6D] concurrent different-key revoke has one lifecycle mutation; audits=$counts operations=$operations"}else{$stats.Partial++;throw "C inconsistent state $state audits=$counts operations=$operations"}

  $race=Race @(
    (Auth $admin "select pg_sleep(.2);select public.admin_revoke_entitlement('$($ents[3])','Concurrent same-key revoke D','p16d-race-same-key-0001');"),
    (Auth $admin "select pg_sleep(.2);select public.admin_revoke_entitlement('$($ents[3])','Concurrent same-key revoke D','p16d-race-same-key-0001');")
  );Inspect $race;Assert-NoLeak $race 'same-key revoke'
  $counts=Scalar "select count(*) filter(where action='entitlement.revoked')||'|'||count(*) filter(where action='fixed_cycle.invalidated')||'|'||count(*) filter(where action='recurring_series.preferred_entitlement_cleared') from public.audit_logs where target_id in('$($ents[3])','$($cycles[2])','$($series[3])')"
  $operations=Scalar "select count(*)||'|'||min(status) from public.entitlement_revoke_operations where operation_key='p16d-race-same-key-0001'"
  $state=Scalar "select e.status||'|'||c.status||'|'||coalesce(s.preferred_entitlement_id::text,'null') from public.fixed_entitlement_cycles c join public.entitlements e on e.id=c.entitlement_id join public.recurring_lesson_series s on s.id=c.series_id where c.id='$($cycles[2])'"
  if($state-eq'revoked|invalidated|null'-and$counts-eq'1|1|1'-and$operations-eq'1|completed'){"D [PASS P1-6D] concurrent same-key revoke has one canonical operation; audits=$counts"}else{$stats.Partial++;throw "D inconsistent state $state audits=$counts operations=$operations"}
